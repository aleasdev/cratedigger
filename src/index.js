...
import { DiscogsClient } from './scripts/discogs.js';
...
overlay.id = 'discogs-overlay';
overlay.className = 'hidden'; // Hidden by default
overlay.innerHTML = `
    <div id="discogs-form">
...
const listsContainer = document.getElementById('discogs-lists');
const btnClose = document.getElementById('discogs-close');
const btnTrigger = document.getElementById('discogs-trigger');
...
async function loadData(records) {
  ...
  // Load into Cratedigger
  cratedigger.loadRecords(records, true, () => {
    // bindEvents is already called for initial load, no need to re-bind unless needed?
    // Actually cratedigger.js binds mouse events in init, but UI button events in bindEvents() here.
    // We should ensure events are bound. Using a flag or just letting it re-bind (might duplicate listeners if not careful).
    // The original replace removed bindEvents call on init. I should restore it.
    // The current structure binds click events to DOM elements. If we re-run bindEvents, we add MORE listeners.
    // We should only bind events once.
  });
}
...
btnCollection.addEventListener('click', async () => {
  ...
  btnCollection.innerText = 'Loading...';
  try {
    const records = await discogsClient.fetchCollection(username);
    loadData(records);
  } catch (err) {
    console.error(err);
    alert('Error loading collection. Check console.');
    btnCollection.innerText = 'Load Collection';
  }
});
...
btnLists.addEventListener('click', async () => {
  ...
  btnLists.innerText = 'Fetching...';
  listsContainer.innerHTML = '';

  try {
    const response = await discogsClient.fetchUserLists(username);
    if (response && response.lists) {
      response.lists.forEach(list => {
        const div = document.createElement('div');
        div.className = 'discogs-list-item';
        div.innerText = list.name + ' (' + list.id + ')';
        div.onclick = async () => {
          div.innerText = 'Loading...';
          try {
            const listData = await discogsClient.fetchListItems(list.id);
            const records = discogsClient.mapListToRecords(listData);
            loadData(records);
          } catch (err) {
            console.error(err);
    ...
    btnLists.innerText = 'Fetch Lists';
  }
});
...
cratedigger.init({
  ...
  onLoadingEnd() {
    // Called when loading animation is done
  }
});
